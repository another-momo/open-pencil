...
- Registries (`registry*.ts`) assemble tool sets. `CORE_TOOLS` is what the built-in agent (ui/marketing chat) sees; `EXTENDED_TOOLS` adds MCP/CLI-only domains (variables, vector ops, codegen, advanced structure). **Prompt files (`src/app/ai/chat/*.md`) may only reference tools in `CORE_TOOLS`** — referencing extended-only tools creates dangling references for the built-in agent. Guarded by `bun run check:tools-consistency` (part of `bun run check`).
