...
#[tauri::command]
pub async fn load_system_font(family: String, style: String) -> Result<tauri::ipc::Response, String> {
    let data = tauri::async_runtime::spawn_blocking(move || load_system_font_blocking(family, style))
        .await
        .map_err(|e| format!("Font load task failed: {e}"))??;
    // Return raw bytes instead of a JSON number array — a 15 MB CJK font would
    // otherwise cross IPC as ~60 MB of JSON and a ~120 MB JS number array.
