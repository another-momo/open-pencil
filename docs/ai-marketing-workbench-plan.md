...
> 最后更新 2026-07-22。三层架构：生图工具（已完成）→ 营销 Agent 模式（Phase 1+2 代码完成，3 轮冒烟测试，实测打磨中）→ 工作台交互改造（构想讨论启动，与 L2 打磨并行）。
...
| L2 营销 Agent 模式 | 营销专用工作流与 prompt | 🔄 实测打磨中 | Phase 1 核心链路 + Phase 2 安全护栏代码完成，3 轮冒烟测试（错误目录见 `marketing-agent-mode-plan.md` §11），第 4 轮回归待跑；后续靠更多真实需求实测迭代 |
| L3 工作台交互改造 | 人机协作界面与流程 | 🔄 构想讨论启动 | 与 L2 打磨并行，产品构想已沉淀：详见 `marketing-workbench-l3-plan.md` |
...
### Layer 3：工作台交互改造（构想讨论启动，与 L2 打磨并行）

L2 三轮冒烟测试证明对话驱动的工作流可行，但也暴露同一模式：**高频、结构化的交互在纯对话里效率低**。更进一步的产品判断：对工作影响最大的是流程形态与工作形态的不匹配（一次性直线流程 vs 反复出图/系列迭代/同一品牌/以发布为终点），由此识别出起点/迭代/终点三个断点。

**产品构想与讨论结论已沉淀至 `marketing-workbench-l3-plan.md`**，核心要点：

- 三类信息模型：宣传内容（每次必答）/ 素材类型（推断为主+显性化）/ 品牌规范（一次配置永久生效），偶有冲突按"硬约束赢、软适配须声明"仲裁
- 概念分层：品牌包 → campaign/制作清单 → 素材类型（规格）→ 素材模板（方案）→ 单次设计
- 多类型诉求用制作清单模型：表达不阻塞、执行串行、状态可见
- 素材类型与素材模板是不同概念：类型是平台规格，模板是排布方案；模板靠"存为模板"积累，不自建库
...
4. **Layer 2 Phase 3**：实测迭代（🔄 打磨中：3 轮冒烟测试完成，第 4 轮回归待跑，持续用真实需求验证）
5. **Layer 3**（🔄 与 L2 打磨并行）：结构化 checkpoint / 品牌包 / 模板选择 / 生图进度 / 导出——具体顺序讨论中
6. **长期**：品牌包深度集成 + 迭代交互优化
