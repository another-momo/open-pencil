...
> **基线**：分支 `rebuild/mode-arch`（起稿于 `docs/zone-governance` worktree @ `7e6752ede`，合并后 §3 数字面板与 §6 由主 agent 按 `5285ad480` 已落地台账重写），merge-base 上游 `88c1077071328b8df68f282543f16e20e97930b4`（2026-08-24，08:24 fix(cli): use Node fs for import command）；upstream HEAD `7964b99ba39e72a3e5e6af1cbd16758daa95b4d5`（2026-09-04 18:44，Merge PR #640）。合并窗口实测：自 merge-base 上游 +180 commits / +758 文件 / +30 154 / -11 281 行（`git diff --shortstat 88c10770..upstream/master`，2026-09-05）。
...
zones.json 的实际顶层字段：`ownedRoots`、`ownedFiles`、`stubs`、`pendingReclass`、`patches`、`deletedPaths`、`upstreamMergeTarball`、`relocations`、`$comment`——共 8 区 + 1 自述。本节按「语义 + 何时进该区」格式列出。
...
**relocations**（搬移台账，2026-09-05 `5285ad480` 落地）：登记「代码搬位置/裁撤但有语义继承者」类重构——`{ oldPath, newPaths[], since, note }` 七条在册。区别于 patch（patch 是上游某 commit 之上叠加 hunk；relocation 是我们自发的位置/语义重排）。checker 配套两条规则：`checkRelocationWatch`（advisory：每次 check:zones 自动对照上游自 merge-base 以来对 oldPath 的活动，命中即提示移植评估，不影响 exit code）+ `checkRelocationsValid`（硬规则：oldPath 非空且不重复）。用法详见 §6。
...
> **核验命令**（2026-09-05）：`node -e "console.log(Object.keys(require('./tools/zone-registry/zones.json')))"`（确认 9 个顶层键，含 relocations）。
...
| deletedPaths         | 129（12 目录前缀 + 117 文件墓碑） | 见 §3.3 目录前缀清单                                                                                     |
...
| relocations          | 7                                 | 已落地（`5285ad480`，2026-09-05）——用法见 §6                                                             |
...
node -e "const z=require('./tools/zone-registry/zones.json');console.log({
  ...
  relocations: z.relocations.length
})"
```

> 实测输出（2026-09-05，主仓 `rebuild/mode-arch` 合并后）：`{ ownedRoots: 15, ownedFiles: 56, patches: 129, permanent: 126, revoked: 3, deletedPaths: 129, dirPrefixes: 12, fileTombstones: 117, stubs: 1, pendingReclass: 0, tarballs: 3, relocations: 7 }`
...
**核验命令**（2026-09-05）：主仓 `rebuild/mode-arch` 合并落账点实测 `bun tools/zone-registry/src/check.ts` exit 0（`[zones] clean: 126 modified (all registered), 694 added (owned), 1130 deleted (all registered), 16 renamed (cross-checked), base 88c10770`）。

## 6. 搬移台账（relocations）使用法

> 2026-09-05 由主 agent 落地（`5285ad480` merge `chore/zone-relocations`）——zones.json 新增 `relocations[]` 七条，check.ts 新增 `checkRelocationWatch`（advisory）+ `checkRelocationsValid`（硬规则）。本节为落地后的当前态描述。

**用途**：登记「代码搬位置/裁撤但有语义继承者」的重构——区别于 patch（上游某 commit 之上叠加 hunk；relocation 是我们自发的位置/语义重排），也区别于 deletedPaths（整功能不要、无继承者）。**它回答的核心问题是合并期的盲区**：上游仍在旧路径上演进（改 bug、加特性），而那些代码已经搬走了——merge 时 git 不会把上游在 `packages/mcp/` 的改动自动对到 `src/app/automation/bridge/server/`，必须靠台账提醒人去评估「上游这份改动要不要吸收到继承者路径」。

### 6.1 schema（已落地）

```typescript
relocations: Array<{
  oldPath: string    // 旧路径（精确文件或以 / 结尾的目录前缀）
  newPaths: string[] // 语义继承者路径（可多目标；纯裁撤无继承者时为 []）
  since: string      // 搬移发生的批次/任务 + 日期
  note: string       // 一句话语义继承关系（哪些迁了、哪些没迁）
}>
```

### 6.2 当前七条台账（2026-09-05）

| oldPath                            | newPaths                                                    | 搬移批次           |
| ---------------------------------- | ----------------------------------------------------------- | ------------------ |
| `packages/mcp/`                    | `src/app/automation/bridge/server/`                         | Batch 2c           |
| `packages/cli/`                    | `packages/core/src/io/`、`tools/visual-oracles/src/render-fixture.ts` | Batch 2b  |
| `src/app/ai/chat/`                 | `src/app/ai/fork/`                                          | Batch 2a           |
| `src/components/chat/`             | `src/components/assistant/`                                 | Batch 2f           |
| `src/app/automation/mcp/`          | （空——纯裁撤无继承者）                                      | Batch 2c           |
| `tests/engine/mcp/`                | `tests/engine/tools/`                                       | Batch 2c           |
| `src/app/ai/chat/system-prompt.md` | `src/app/ai/pi-backend/studio/base.md`                      | T46（副本已退役）  |

### 6.3 RELOCATION_WATCH（advisory，每次 check:zones 自动跑）

`checkRelocationWatch`（[check.ts](../tools/zone-registry/src/check.ts)）在每次 `bun run check:zones` 末尾执行：取 `merge-base(HEAD, upstream/master)..upstream/master` 的 `--name-status -M` diff，凡命中台账 oldPath（目录前缀 startsWith / 精确文件相等 / R 行 from-to 两端匹配）即打印：

```
[zones][watch] RELOCATION: <oldPath> 上游有改动（N 文件）→ 移植评估 → <newPaths>：<note>
```

**advisory 语义**：只提示、永不影响 exit code；`--drift` 模式附逐文件清单；上游不可达（离线）时静默跳过。2026-09-05 首战实测：七条台账全部命中（上游在全部 7 个旧路径上自 base 以来都有活动，合计 28 文件）——合并期该盯哪里，现在每次跑 checker 自动提醒。

### 6.4 RELOCATIONS_VALID（硬规则）

`checkRelocationsValid`：oldPath 非空且不重复——防台账登记腐烂，违反即红（与其他 violations 同等地位）。

### 6.5 移植决策落点

watch 命中后 owner 逐条判定：

- **判「移植」**：将上游对应 commit 的 diff 吸收到 `newPaths`（cherry-pick 或手工搬运），commit message 引用台账条目；
- **判「不适用」**：在 `records/topics/<topic>.md` 追加「<oldPath> 上游改动不吸收：<理由>」，台账条目本身保留（下次 watch 仍会报，但判定结论有档案可查）；
- **newPaths 为 [] 的纯裁撤条目**（如 `src/app/automation/mcp/`）：watch 报告仅作信息——上游继续演进该路径天然不吸收，除非 owner 拍板复活该功能。

**登记纪律**：zones.json 单一写者是主 agent（子代理 worktree 产出由主 agent review 后合入落账）；台账条目随搬移批次入库，沿用 CHECK_HOOKS_SKIP 落账先例时须在 commit message 写明前因后果。
