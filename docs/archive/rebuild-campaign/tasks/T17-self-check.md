...
6. **渲染面**：`ConversationSnapshot`（`sessions/conversation.d.ts:371-409+`）关键字段：`nodes: readonly ConversationNode[]`（seq 作 key 的联合：user / assistant / steering / context / model-retry / turn-error / turn-max-tokens / tool-result / command / compaction / unknown（判别值实测 `kind:'compaction'`，conversation.d.ts:200；原记 "compaction-summary" 有误，2026-08-23 核验修正；联合定义见 :264））、`partial: PartialAssistant | null`（{turn, step, blocks: AssistantBlock[]}，:295）、`running: boolean`、`promptError`、`hasMore/loadingOlder/openState/openError/queue/pending/removed/composerPhase`；AssistantBlock 联合：text / reasoning / image / tool-call{callId,name,argsRaw} / other（:30-46）
