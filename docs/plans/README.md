...
| `l2-context-engineering.md` | L2 子规划：上下文工程——prompt 装配、Manifest、窗口化（**已经评审，修订意见见 review，实施时合并**） | 半持久 |
...
| L2 上下文工程（子规划） | 📋 已评审未实施 | 修订后按 B → C → A/D 执行（A0 已解散、砍 Phase E；Phase C 开工当天量新鲜基线） |
...
2. **L2 第 4 轮护栏场景回归**（待跑）：护栏修改/删除/有意修改/类型切换 + CP3 图片来源 + 用户素材识别 + 叠字 hero 产出 + look 去重行为（用例见 `knowledge/error-catalog.md`）
3. **L2 上下文工程 Phase B（修订版）**：单源化 Manifest + 推导制状态 + per-rootFrame 键控
4. **L2 上下文工程 Phase C（修订版）**：窗口化（保留用户消息 + media elision 语义，R4-3 实测确认 P0；开工当天用标准用例量新鲜基线）
5. **L2 上下文工程 Phase A/D**：prompt 拆分 + patterns 注册表
