...
- 2026-08-11：第二次合并完成（`merge/upstream-2026-08-v2`，两段式：`c82daea5` / `d154cdff`）——67 commits（`de8578c4..c29654cd`，v0.14.0 发布 + 存储工作区预览 + 聊天面板模型切换 + FIG 保真一批 + acronym casing 重构 159 文件）。第一段（合到 `4d6ddd3f`，重构前）**零冲突**；第二段 6 个冲突文件全部机械级，半天内完成。处置明细：
  ...
  - CI 三轮收敛（首轮 acronym lint 红 / 次轮 type-shapes 红 / 第三轮全绿 run 31498060499）：upstream 新 lint 规则 `no-mixed-case-acronym-identifiers` 抓出 fork 12 个旧式命名（`appendToBriefAiZone`/`visionApiKey`/`imageGenApiKey` 等 51 处，22 文件，`9a6a67ac` 一并改名）；同 commit 修复存量 lint 错误 3 个（`compose-backdrop.ts` 复杂度 25→抽 `validateCanvasSize`/`validateHeroBleed`、optional-chain、poster 测试空函数）；fork 的 `RGB` 与 upstream 新测试 `OracleColor` 形状撞车，改 `type RGB = Pick<culori Rgb, 'r'|'g'|'b'>` 复用了事（`8ec4da4c`）。**教训：本机（Windows/autocrlf）跑 `bun run format` 会产生 271 个幻影 modified（内容与索引逐字节一致），format:check 以 CI 为准，本机不跑。**
...
- 2026-08-13：渲染器 imageCache 加 LRU 字节预算——web dev 跑 marketing 长图会话反复 OOM 崩页的根因治理。根因链：①`imageCache`（解码后全分辨率位图 + mipmap 副本，常驻 WASM 堆）只有 set 没有淘汰，唯一清理是 renderer 销毁；②fork 的 agent 工具把产图速度从人手速提到机器步速（generate_image 每次新 hash、重生成 ×N、历史备份留存）；③工具侧导出渲染（`renderSceneToCanvas`，`pipeline.ts` 人为关视口剔除）把子树内所有图灌进解码缓存，与用户是否观看无关。修复：`fills.ts` `applyImageFill` 加 LRU touch + 插入时按字节预算（默认 512MB，`DEFAULT_IMAGE_CACHE_BUDGET_BYTES`，renderer 字段可覆写）淘汰最旧条目并 `delete()`；淘汰后下次渲染从 `graph.images` 透明重解码，**渲染输出不变**。
  - 改动文件：**`packages/core/src/canvas/fills.ts`、`renderer.ts`、`renderer/lifecycle.ts` 自此进入 M 类低危清单**（此前零分歧）——hunk 集中在 `applyImageFill` 缓存存取点与两个新字段，upstream 若动图片填充/缓存逻辑需语义级合并。测试 `tests/engine/render/canvas/image-cache.test.ts` 为 A 类新增。
  - 明确否决的替代方案：解码时按显示尺寸降采样——`imageCache` 被屏幕渲染与高倍导出（同一 renderer 实例）共用，降采样会糊掉缩放查看与 2x/4x 导出，属功能回归。
  - 已知边界：WASM 堆只涨不缩，淘汰是封顶不是回吐；`graph.images` 编码字节仍无引用计数回收（牵扯 undo 快照/协作删除回放，留待后续单独做）。
  - 验证：新增单测 4 条全绿（预算淘汰/LRU 顺序/淘汰后重解码/销毁重置）；raster-export、image-fill、renderer-lifecycle、culling、retained-backing 定点全绿（`cache.test.ts` 经 stash 对照证实为 HEAD 既有失败，fixture 解析问题，与本改动无关）；tsgo 干净；oxlint 本机内存临界跑不起来（既有环境问题，同 2026-08-12 记录）。
