...
| 2.3 | 多 design 且 latest 失效时（`registry.ts:75-86` 刻意保留 stale 候选），`look.ts:50-55` 的 error 只列候选不区分死活——唯一活的设计本可识别，却让用户手动选 | error 候选列表按 `graph.getNode(design.rootFrameId)` 标注 `(live)` / `(root deleted)` | ~6 LOC + 测试 |
