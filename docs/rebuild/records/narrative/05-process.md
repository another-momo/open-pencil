...
## 修正-N · 文档纪律轮（头部纯化 + 树状图修正 + tracker 结构段规范化 + 真源互换同步 + 附录 B.1 登记规则）

- **类型**：修正（按对象：05-process.md）
- **时间**：2026-08-25
- **依据**：D31（04/05 文档纪律轮 + 真源互换，owner 2026-08-25）；决策登记见 [records/topics/docs-governance.md](../topics/docs-governance.md)
- **内容**：
  1. **头部纯化**：状态字段长 changelog 化时间纯化（状态：已核验 | 时间：2026-08-25 | 核验人：主 agent）——原字段含五次修订史全量堆砌
  2. **§2 树状图**：tracker 行描述改「阶段门 + 任务表（当前任务行 + 已收口分组行）+ 记录索引」；插入 runbook-github-push.md 行（补漏）；「tasks/_index.md # 任务表（镜像 tracker.md §2）」→「# 逐任务索引真源（§2 任务清单含三件套路径列）+ 实录归档（§6）」
  3. **§2「关于 tracker.md 拆分」段规范化**→「tracker.md 结构」段：保留拆分史一句 + 补 D31 两区定义（逐任务索引真源=_index.md §2，tracker=当前任务行+分组行）
  4. **§3.2 任务表真源行改口**：「Tracker.md：任务表的真源…_index 作为辅助镜像同步」→「任务表（两区，D31）：逐任务索引真源 = tasks/_index.md §2…tracker.md §2 是阶段门视角摘要」
  5. **§4.11 任务表路径列行改口**：真源互换同步（D31）
