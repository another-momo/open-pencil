...
## T37 修正-N（2026-08-28） · T37 行追加 + 本档案停更说明（D36）

- **改动**：tracker.md §2 追加 T37 行（决策批次登记，状态 🔄）
- **停更说明**：自 2026-08-28 起 tracker.md 从 D14「修改触发」强制口径豁免（owner 拍板，D36，规则文见 05-process.md §4.10 豁免条款）——**本档案保留停更，不再强制逐 commit 追加**；tracker.md 的后续变更历史以 git 历史为准。本条为本档案在强制口径下的最后一条义务性追加
