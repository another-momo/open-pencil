...
- **第二轮红 run id 补录**：上文 §V6 只记了首轮红 run 32655127504 与终绿 run 32656186119，整改 commit c7a0a44c 自身的 CI run **32655585170（failure）**未记 id。溯源：`gh run list -R another-momo/open-pencil --branch rebuild/pi --limit 30`（2026-08-25）——c7a0a44c 对应 run 32655585170（2026-08-23 17:40Z，failure），随后的 7431f9f4 对应 32656186119（success）。即 T21 实际经历两轮红（format/dupes 红 → 整改 commit 仍红 → 二次整改转绿），非「一轮红即翻绿」。
