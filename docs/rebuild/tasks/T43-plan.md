...
| D-d | 校验面 v1 | ✅ | ①通用：frontmatter 可解析且为 map、`id` 必填且与文件名一致（机读 id：小写字母/数字起头 + 连字符/下划线分段——**kebab/snake 双收**，实测修正：S2 §4/§5 自带示例 `ecommerce_detail`/`watercolor_poster_v3` 均为 snake_case，纯 kebab 校验会拒绝规格自身示例）、`label` 必填 ②workflow：`types` 必填（列表或 `none`）；列表每 type 需 id/label/size（`WxH` 固定或 `Wx` HUG），且**同名正文小节存在且非空**（S2 §4 必填体节）；`step_budget` 若存在须正整数 ③profile：必需小节（Fixed system/Variable system/Anti-identity/Tone/Recipe）非空或节内显式 `no-op`（S2 §5）；`applicable_to` 引用完整性（mode id 存在于注册表或 `general`）；hex 色值检查（v1 启发式：仅报长度 5/7 全 hex 或长度 6/8 含非 hex 字符——短编号如「#1」不误报）；**字体白名单**经 `fontRegistryEntry` 校验（core 引用链已实证存在且静态 import 无副作用：2026-08-30 `bun -e` 实测，§6 风险行排除） ④base：至多一份生效（用户覆盖内置）；全缺 → failures 记整体缺失态 |
