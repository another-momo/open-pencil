...
| V2 保真核验复跑 | 过 | `bun workbench/verify-t44-migration-fidelity.mjs` → 21 pass / 0 fail；人工抽查 v3 Recipe 与 config.yaml:274-301 源段逐字一致，唯一偏差 = 步骤 1 列表标记 `1.（`→`1. （`（脚本 NORMALIZE 表与自检 C2 登记吻合） |
