...
|---|---|---|
| V1 端点实证 | 过 | 复跑 `bun workbench/probe-t45-old-route.mjs` → 旧路径 404 / 新路径 200（modes=[general(0), longform(3)]、profiles=3、failures=1 相对路径）；另写独立探针 dump 全 JSON（workbench/verify-t45-manifest-dump.mjs）：top-level 仅 modes/profiles/failures、profile 三无 body/markdown 键、全文无盘符泄漏；复跑 prompt-assembly-smoke → 29/29 |
...
| V6 回归对照 | 过 | 复核 workbench/t45-regression-run.log 尾 78 fail/2660/434 文件；`diff /tmp/t44-fails.txt workbench/t45-failures.txt` 仅 +1 行（fig export subgraph flake）；T45 文件 grep 零命中；亲复跑 `bun test tests/engine/io/subgraph.test.ts` → 4/4 |
...
- 核验方落的探针文件 workbench/verify-t45-manifest-dump.mjs（+ .json 输出）作为 V1 证据随收口 commit 登记。
