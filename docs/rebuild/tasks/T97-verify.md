...
## 合并落账后主 agent 复核（2026-09-05，修正-2 对应）

- **背景**：分支合并瞬间两处前提变化——①`77e32774a` 的 zones.json 登记债由主 agent `925a0f7a4` 补票入库（初版顺手修成为重复）；②`5285ad480` 合并 `chore/zone-relocations`，relocations 字段实际落地
- **处置**：§6 由主 agent 从「未落地设计草案」重写为已落地机制描述（真实 schema / 七条台账 / RELOCATION_WATCH advisory 首战七条全命中 28 文件 / RELOCATIONS_VALID 硬规则）；§2 顶层键 7→8 区；§3.1/§3.4 数字面板刷为主仓合并后实测（deletedPaths 129 = 12 前缀 + 117 墓碑）；records 追加修正-2
- **复核门禁**：check:zones clean（126/699/1130/16，RELOCATION_WATCH 七条正常提示）+ check:docs 45/45 + check:bindings 全绿；check:tasks 要求本 commit 挂 `task: T97` 指针（本段即配合登记的更新）
- **遗留**：初版 verify 所述「owner 触发补办的独立 subagent 核验」仍待派单，状态不变（🟡 待验收）
