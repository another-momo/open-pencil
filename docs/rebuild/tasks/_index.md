...
| [T34](../tasks/T34-plan.md) | 上游合并第三轮 | octopus 8 commits（`88c10770`→`0f981ff2`，含 diagnostics + crash recovery + Portless 隔离）；24 冲突三类解（modify/delete 6 取我们删侧 + dialogs.json 8 [zh-cn 保留 pi 段 / 7 个语种删] + content 10）；host.ts spawnBridge 决策注记（不跟 DISCOVERY_PATH 隔离）；AppTextButton 误删纠正 + zones 误判纠正（checkDeletedAbsent 已覆盖） | ✅ 已完成（V1-V8 全过「可以收口」+ 11 项门禁绿 + smoke:pi 80 passed；远端推送阻塞——环境网络层 github.com 不通，待 owner 协助） | [T34-plan](../tasks/T34-plan.md) | [T34-self-check](../tasks/T34-self-check.md) | [T34-verify](../tasks/T34-verify.md) |
| (后续 task 按顺序登记)      | —                          | —                                                                                                                       | —                                                                                                                                            | —                                | —                                            | —                                    |
