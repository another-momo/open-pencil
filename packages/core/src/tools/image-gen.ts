...
export const generateImage = defineTool({
  ...
  mutates: true,
  description:
    'Generate or edit an image via an OpenAI-compatible image API (gpt-image-2) and place it on the canvas as an editable image node. Pass a JSON array for batch. Each item: {prompt, width, height, quality?, output_format?, output_compression?, id?, background?}. Omit id to create a new image frame. Pass the id of an existing node to target it: a node with an IMAGE fill is edited image-to-image (its pixels are uploaded); a node WITHOUT an image fill is filled directly with the generated image — this works on leaf shapes (Rectangle/Ellipse placeholders) AND on Frames, where the image becomes the frame background while children are kept (the standard way to build a text-over-image hero). For local edits, describe the target region inside the prompt (e.g. "add the logo in the top-right corner"). gpt-image-2 only accepts a fixed set of sizes (1024x1024, 1536x1024, 1024x1536, 2048x2048, 2048x1152, 3840x2160, 2160x3840) — requested width/height are auto-mapped to the nearest allowed size and reported in `note`.',
  params: {
    requests: {
      type: 'string',
      description:
