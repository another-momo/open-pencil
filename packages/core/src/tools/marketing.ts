...
export const setupMaterialTypeTool = defineTool({
  ...
  mutates: true,
  description:
    'Set up a marketing design from a material type. Creates the root frame at the design size declared in the active brand config, and returns the resolved size. Material types come from the active brand config (see `/v1/brand/types`) — available ids are shown in the system prompt and in the error returned for unknown ids. Continue/repair is PAGE-scoped: calling again with the same id on the same page adopts that page\'s design (the result then says adopted: true with its existing child count — confirm with the user whether to continue it or start over); the same id on a different page, or with mode: "new", always creates a fresh design alongside existing ones — one document can host many designs, and adoption never crosses pages.',
  params: {
    id: {
      type: 'string',
      description:
        'Material type id from the active brand config, e.g. "wechat_moments", "product_long". Use "custom" with width+height for sizes no preset covers.',
      required: true
    },
    mode: {
      type: 'string',
