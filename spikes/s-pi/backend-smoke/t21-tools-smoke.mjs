...
for (let attempt = 1; attempt <= 3; attempt++) {
  ...
  const tools = round.chunks
    ...
    .map((c) => c.toolName)
  if (tools.includes('describe') && tools.includes('render')) break
  console.log(`  … 第 ${attempt} 次未凑齐 describe+render（实际：${tools.join(',') || '无'}），换 session 重试`)
}
...
check('render 产出含节点 id（桥真实执行）', (() => {
  const renderCallId = [...nameByCallId.entries()].find(([, name]) => name === 'render')?.[0]
  const out = round.chunks.find(
    (c) => c.type === 'tool-output-available' && c.toolCallId === renderCallId
  )
  return typeof out?.output?.id === 'string'
})(), JSON.stringify(round.chunks.filter((c) => c.type === 'tool-output-available').map((c) => c.output)).slice(0, 200))
check('finish(stop) 收尾 + [DONE]', round.chunks.at(-1)?.type === 'finish' && round.sawDone)
