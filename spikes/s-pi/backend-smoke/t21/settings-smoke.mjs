...
check('前置：OPENROUTER_API_KEY 在脚本环境（只经页面输入框传输）', KEY.length > 0)
...
check('初始存储态：auth.json 无 openrouter 条目', (() => {
...
try {
  ...
  await page.locator('[data-test-id="app-settings-dialog"]').waitFor({ timeout: 10000 })
  ...
  const routerRow = page.locator('[data-test-id="pi-provider-row"][data-provider-id="openrouter"]')
  ...
  check('auth.json 落盘且为 pi 格式（key 匹配，不打印）', (() => {
    if (!existsSync(authPath)) return false
    const doc = JSON.parse(readFileSync(authPath, 'utf8'))
    return doc?.openrouter?.type === 'api_key' && doc?.openrouter?.key === KEY
  })())

  // design 模型指派：openrouter + 首个模型 + 保存
  await page.locator('[data-test-id="pi-design-provider-select"]').selectOption('openrouter')
  const modelSelect = page.locator('[data-test-id="pi-design-model-select"]')
  await modelSelect.waitFor({ timeout: 5000 })
  const firstModel = await modelSelect.locator('option').first().getAttribute('value')
  check('design 模型下拉已随 provider 填充', typeof firstModel === 'string' && firstModel.length > 0)
  ...
  // configured，故清理断言只看 auth.json 文件）
  await page.locator('[data-test-id="provider-settings-trigger"]').click()
  await page
  ...
  check('清理：UI 清除 key 后 auth.json 不再含 openrouter', (() => {
    if (!existsSync(authPath)) return true
    return JSON.parse(readFileSync(authPath, 'utf8'))?.openrouter === undefined
  })())
