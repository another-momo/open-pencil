 ...
 * 运行：bun spikes/s-pi/backend-smoke/t22/bind-smoke.mjs [base=http://localhost:1420]
...
page.on("pageerror", (err) =>
  console.error(`[pageerror] ${String(err).slice(0, 200)}`),
);
  ...
  return page.evaluate(() => {
    ...
    return (
      root?.pluginData?.find(
        (e) =>
          e.pluginId === "openpencil.ai" && e.key === "openpencil.ai/docId",
      )?.value ?? null
    );
  });
}
...
try {
  ...
  const prefix = docId
    ? `doc-${createHash("sha1").update(docId).digest("hex")}`
    : null;
  check(
  ...
  if (prefix) {
    ...
    await page.evaluate(() =>
      window.openPencil?.getStore?.()?.persistRecoveryNow?.(),
    );
    ...
    const docIdAfterRestore = await activeDocId();
    check(
      "② 恢复还原后 docId 同一身份（快照通道持久化）",
      docIdAfterRestore === docId,
    );
    ...
    if (backfilled) {
      ...
      check(
        "③ 旧会话在后端 index.json 归档保留",
        Boolean(indexNow[seedSessionId]),
      );
    }
    ...
    await page.waitForFunction(
      () =>
        window.openPencil
          ?.getStore?.()
          ?.state?.documentName?.includes("circle-text"),
      undefined,
      { timeout: 20000 },
    );
    ...
    check(
      "④ 第二文档会话前缀不同（族谱隔离）",
      SESSION_ID_RE.test(second?.sessionId ?? "") &&
        !second.sessionId.startsWith(`${prefix}-`),
      second?.sessionId,
    );
    check(
