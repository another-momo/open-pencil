...
function supportsAnthropicCaching(
  providerID: AIProviderID,
  modelID: string,
): boolean {
  return (
...
export function createToolLoopTransport({
  ...
  const cacheProviderOptions = supportsAnthropicCaching(
    providerID,
    effectiveModelID,
  )
...
export function createChatSessionManager({
  ...
  let overrideTransport:
    ((store: EditorStore) => ChatTransport<UIMessage>) | null = null;
  ...
  async function createActiveHarnessTransport() {
    ...
    const apiKey = await resolveModelConnectionAPIKey(
      runtime.role.connection.id,
    );
    ...
    const transport = new HarnessChatTransport(
      `tab-${getActiveTabId()}-${runtime.role.profile.id}`,
      {
        ...
        settings: {
          ...
          permissionMode:
            runtime.role.profile.harnessPermissionMode ?? "allow-edits",
        },
        instructions: SYSTEM_PROMPT,
  ...
  async function createTransport(store: EditorStore) {
    ...
    if (runtime?.kind !== "direct") {
      throw new Error(
        "The Design model is not configured for direct API access",
      );
    }
  ...
  async function ensureChat(): Promise<Chat<UIMessage> | null> {
    ...
    if (!chat || transportDirty || currentChatStore !== store) {
      ...
      if (isHarnessProvider.value)
        transport = await createActiveHarnessTransport();
