...
function findDocIdEntry(store: EditorStore): string | null {
  ...
  return (
    root?.pluginData.find(
      (entry) =>
        entry.pluginId === PI_DOC_NAMESPACE && entry.key === PI_DOC_ENTRY_KEY,
    )?.value ?? null
  );
}
  ...
  store.graph.updateNode(store.graph.rootId, {
    pluginData: [
      ...(root?.pluginData ?? []).filter(
        (entry) =>
          !(
            entry.pluginId === PI_DOC_NAMESPACE &&
            entry.key === PI_DOC_ENTRY_KEY
          ),
      ),
 ...
 */
export async function loadPiChatHistory(
  store: EditorStore,
  ...
  try {
    ...
    const res = await fetch(
      `/api/pi/history?docKey=${encodeURIComponent(prefix)}`,
    );
 ...
 */
export async function getPiRequestContext(
  store: EditorStore,
): Promise<PiRequestContext> {
