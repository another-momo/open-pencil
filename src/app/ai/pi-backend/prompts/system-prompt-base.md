<!-- T46（S4 W1/T-A5）互指：本文已全文转写至 src/app/ai/pi-backend/studio/base.md（T47 起本文替代 chat/system-prompt.md 成为转写源；T49 起 base.md 为纯转写，不承载显式纪律段）——变更本文须同步 base.md；每回合组装接入（W2/W3，S2 §6）后本文退役。同步核验：node tools/rebuild/src/verify/t46-base-fidelity.mjs（剥 frontmatter 与 T46 头注后零 diff）。 -->
