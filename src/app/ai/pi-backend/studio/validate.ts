...
/** reference path 形态问题（null = 合法）：扩展名白名单（P2-3a），禁 `..` / 绝对路径 / 盘符 / UNC。
 *  入参须已做反斜杠归一（`\\` → `/`）；读取侧（load-reference.ts）同口径再查一次（纵深防御） */
 ...
 * 存储归一后形态（索引注入与 load_reference 匹配同口径）。
 */
