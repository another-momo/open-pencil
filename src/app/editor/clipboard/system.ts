...
// T31：eslint(complexity) 21>20——上游 7a311677 引入的降级链单函数超限，
// 按纯度最小重构抽 helper（行为不变；上游本体未做此拆分时我们维护此补丁）
async function tryModernClipboardWrite(html: string, plainText: string): Promise<boolean> {
  if (
...
export async function copySelectionToBrowserClipboard(store: EditorStore): Promise<boolean> {
  try {
    ...
    if (await tryModernClipboardWrite(html, plainText)) return true
    if (tryExecCommandCopy(html, plainText)) return true
