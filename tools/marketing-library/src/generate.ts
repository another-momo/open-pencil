...
export function buildDefaultLibraryGraph(): SceneGraph {
  ...
  graph.createNode('TEXT', promo.id, {
    ...
    text: [
      ...
      '## Fixed system (never break)',
      '',
      "- Full-scene illustrated backdrop via the shared backdrop flow (compose_backdrop); sections below the hero sit on ONE white rounded content island (corner 24–32px, padding 32–40px) — but never a bare expanse: group the island's content into light-tint info blocks (the design's accent color at 6–10% alpha) with title bands and dividers from the decoration vocabulary. No flat per-section color blocks outside the island.",
      ...
      '',
      '1. (Phase 2 skeleton) Render a transparent Frame named `HeroContent` as the first flow child of the root frame, h=750, w=canvas width — and place the hero TITLE text inside it NOW (default centered-stack), never in Phase 3: the title must exist before any image is generated so the scene is composed around it. At the same time, write the decoration plan into the skeleton: which sections get a stroke-band title, which accent stickers, and where they go — the sheet in step 4 serves THIS plan. Content sections live on the white content island (rounded 24–32px card) or transparent frames — no per-section color blocks.',
