...
export function buildDefaultLibraryGraph(): SceneGraph {
  ...
  graph.createNode('TEXT', watercolorV3.id, {
    ...
    text: [
      ...
      '',
      '1.（Phase 2 骨架）渲染透明 Frame `HeroContent` 作为 root 的第一个 flow 子节点（高=你的 hero 高度选择，宽=画布宽），并在其中完成标题排版——文案、字号、位置、字重现在定；**颜色先不定**，临时用深色即可（第 5 步由色票定最终色）。所有内容 section 保持透明填充。',
      ...
      '3. 调用 `generate_image`：目标 = 画框 id，`references: [{"id": 画框id, "composite": true}]`。参考图与成品同尺寸、文字在真实位置——生图 AI 看着文字构图。prompt 要求：文字覆盖区平静、低细节、影调明确偏浅或偏深（写明你的选择）；底部 ~100px 保持平静（fade 区）；画面内不得出现任何文字（参考图中的文字仅是位置参照）。水彩风格：柔和叠染、单一意象、大量留白。',
      ...
      '5. 调用 `derive_palette({ seed: <第 4 步返回的 hero_color>, harmony: <本设计选定的 harmony> })`——把返回色票落实：hero 标题设为 `ink.onLight`（标题带偏浅时）或 `ink.onDark`（偏深时），以 checks 中对比度过关者为准；后续 section 用色全部从色票角色取（正文 ink.onLight、安静的底 ground/neutrals、accent 克制使用）。note 里的警告必须遵守。',
