...
export function buildDefaultLibraryGraph(): SceneGraph {
  ...
  graph.createNode('TEXT', promo.id, {
    ...
    text: [
      ...
      '## Fixed system (never break)',
      '',
      '- Full-scene illustrated backdrop via the shared backdrop flow (compose_backdrop); content sits on a white rounded content island (corner 24–32px, padding 32–40px). No flat per-section color blocks — a card background is either clean white or a generated texture panel from the same decoration pipeline (never a flat UI fill).',
      ...
      "- Section titles sit on a marker-brush stroke band from the decoration sheet: place the band as an image node BEHIND the title text (lower z-order inside the title's frame), width = title width + ~48px padding, height follows the asset's native aspect — never stretched, never a clean rectangle. A clean rounded rect behind a title reads as UI, not promo.",
      ...
      "4. Generate the DECORATION SHEET with the hero as `references` (the hero is the style anchor for every decoration): one green-screen image (solid #00FF00 background, no shadows, hard edges, 3x3 grid, no text) containing this design's sticker vocabulary — marker-brush stroke banner, starburst badge, arrow, tape label, sparkle, plus event-specific pieces. Then `cutout({ id: <sheet node>, expected: <number of pieces on the sheet> })` — assets come back in reading order with native sizes.",
