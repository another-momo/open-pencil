...
export function buildDefaultLibraryGraph(): SceneGraph {
  ...
  graph.createNode('TEXT', watercolor.id, {
    ...
    text: [
      ...
      '',
      '## Backdrop recipe',
      'A single hero image at the top of the canvas, with one gradient rectangle sitting below it (in auto-layout sibling order) that runs to the canvas foot. **No absolute positioning**: the overlay is an auto-layout child of the root frame with `w="fill"` and a concrete `h`, inserted at the right index so it lands between hero and content sections. Using `x`/`y` on the overlay breaks the auto-layout flow and silently covers content sections below.',
      '',
      'Three linear-gradient stops:',
      '',
      '- Position 0 (top of overlay): `#FFFFFF` at alpha 0 — fully transparent so the hero above shows through with no visible seam.',
      '- Position `100 / overlayHeight` (where the hero bottom edge lands in the overlay): the hero theme color at alpha 1 — fill this stop with the hex returned by `sample_hero_color({ id: hero.id, direction: "bottom" })`. Do not invent a theme color; the tool averages actual hero pixels.',
      '- Position 1 (canvas foot): `#FFFFFF` at alpha 1 — opaque white so content sections below the overlay sit on a clean surface.',
      '',
      ...
      '',
      '**Verify with `look` after rendering the overlay.** If content sections below the overlay are not visible, the overlay is wrongly placed (absolute x/y) or the bottom stop is the hero color instead of `#FFFFFF`. Re-render with the corrected recipe — do not "fix" by moving other sections.',
      '',
      '## Tone',
