/**
 * T46（S4 W1/T-A5）保真核验：studio/base.md 与 system-prompt.md 的逐字一致性。
 *
 * 等式：strip(base.md) === strip(system-prompt.md)
 *   strip = 剥 frontmatter（base 侧）→ 剥 T46 红线补洞段标记块 → 全局剥 T46 注释行
 * 即：除补洞段与双源头注外，base.md 对 system-prompt.md 零偏差（C1 硬卡口）。
 * 另断言：标记 begin/end 各恰好一次、frontmatter id=base、补洞段含四红线与
 * 修辞事实标注的语义锚点（防空段/标记在而内容被掏空）。
 *
 * 运行：bun workbench/verify-t46-base-fidelity.mjs（仓根）
 */
...
const src = readFileSync(join(repoRoot, 'src/app/ai/chat/system-prompt.md'), 'utf8')
...
check(
  '双源头注两文各一（base 双源声明 / system-prompt 互指；标记块注释不计）',
  (base.match(/^<!-- T46（S4 W1\/T-A5）/gm) ?? []).length === 1 &&
    (src.match(/^<!-- T46（S4 W1\/T-A5）/gm) ?? []).length === 1
)

const block = base.match(BLOCK_RE)
check('补洞段含四红线语义锚点', !!block && ['Facts are never invented', 'explicit confirmation', 'single undo unit', 'never silently worked around'].every((s) => block[1].includes(s)))
check(
...
check(
  '逐字保真：剥除后 base.md === system-prompt.md（零 diff）',
